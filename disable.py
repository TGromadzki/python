# Script for disabling users from multiple Linux/AIX servers (Python 2.7)
...
def disableFunc(hostname, ipaddress, systemInfo, username, password, idsTable):
    try:
        session = pxssh.pxssh()
        ...
        if systemInfo == 'LNX':
            for elem in idsTable:
                session.sendline('passwd -l ' + str(elem))
                #session.expect(['Success', pexpect.EOF, pexpect.TIMEOUT])
        elif systemInfo == 'AIX':
            for elem in idsTable:
                session.sendline('chuser account_locked=true ' + str(elem))
                #session.expect([pexpect.EOF, pexpect.TIMEOUT])
        else:
            ...
            for iterateServersCsv in serversCsv:
                for key in slownik.iterkeys():
                    if iterateServersCsv['server'].lower() == key.lower():
                        ...
                        disableFunc(hostname, ipaddress, systemInfo, username, password, slownik[key])
